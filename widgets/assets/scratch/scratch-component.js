class WgtScratchCard extends HTMLElement {
  ...
  async prepareScratchWrapper(target) {
    ...
    const appendWrapper = () => {
      ...
      this.startScratchCard(`#${scratchWrapper.id}`, currentWidth, currentHeight, coverImage, this._overlayOn, clearZoneRadius);
    };
  ...
  async startScratchCard(containerSelector, width, height, coverImageSrc, mode, clearZoneRadius) {
    try {
      ...
      if (coverImageSrc) {
        ...
        await new Promise((resolve, reject) => {
          img.onload = () => {
            ...
            drawImageScaledAndCentered(offscreenCtx, img, width, height);
            resolve();
          };
          img.onerror = reject;
        });
      ...
      console.error('[wgt-scratch-card] ScratchCard init error:', err.message);
    }
  }
...
function drawImageScaledAndCentered(ctx, img, canvasWidth, canvasHeight) {
