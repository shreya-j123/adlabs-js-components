class WgtScratchCard extends HTMLElement {
  // static _overlayWidgetMap = new Map(); 

  constructor() {
    super();
    this._target = null;
    this._scratchInstance = null;
    this._scratchWrapper = null;
    this._observer = null;
    this._overlayOn = '2';
    this._ScratchTextElement = null;
  ...
  render() {
    ...
    const scratch_id = `scratch-${Math.random().toString(36).substr(2, 5)}`;
    if (!this.adWidgetId) {
      this.adWidgetId = this.getAttribute('ad_widget_id') || scratch_id;
    }
    this.widgetId = this.getAttribute('widget_id') || null;
    this.widgetOrder = this.getAttribute('order') || "ScratchToReveal";
    this.setAttribute("id", this.widgetOrder);
  }

  waitForTargetAndInit() {
    WidgetController.waitForTargetAndInit(this, this.prepareScratchWrapper);
  }

  async prepareScratchWrapper(target) {
    const coverImage = this.getAttribute('cover_src');
    const coverImageRendition = this.getAttribute('cover_image_rendition');
    const clearZoneRadius = 80;

    if (!target) return console.warn('[wgt-scratch-card] Missing target or cover image');

    await new Promise(r => requestAnimationFrame(r));
    await new Promise(r => requestAnimationFrame(r));

    if (target.offsetWidth < 1 || target.offsetHeight < 1) {
      return console.warn('[wgt-scratch-card] Target has no visible size yet.');
    }

    if (this._scratchWrapper && this._scratchWrapper.isConnected) {
      console.warn('[wgt-scratch-card] Scratch wrapper already exists. Skipping.');
      return;
    }
    ...
    const appendWrapper = () => {
      ...
      if (currentWidth < 1 || currentHeight < 1) {
        return console.warn('[wgt-scratch-card] Target has no visible size after ad-render');
      }

      const scratchWrapper = document.createElement('div');
      scratchWrapper.classList.add(`scratch-wrapper`);
      scratchWrapper.id = `js--sc--container-${Date.now()}`;
      Object.assign(scratchWrapper.style, {
      ...
      this.appendChild(scratchWrapper);

      const existingWrapper = document.querySelector(
        `wgt-scratch[id="${this.widgetOrder}"]`
      );
      if (existingWrapper) {
      ...
      this._scratchWrapper = scratchWrapper;

      this.addScratchTextOverlay(scratchWrapper);
      this.startScratchCard(`#${scratchWrapper.id}`, currentWidth, currentHeight, coverImage, coverImageRendition, this._overlayOn, clearZoneRadius);
    };

    // A small delay to ensure the target is fully rendered and sized
    setTimeout(() => {
      if (!this._scratchWrapper) { // Only append if no wrapper already exists from a rapid re-render
        appendWrapper();
      }
    }, 300);
  }

  addScratchTextOverlay(parentEl) {
    this._ScratchTextElement?.remove();
    this._ScratchTextElement = null;

    const ScratchTextHtml = this.getAttribute('text');
    const ScratchInstruction = this.getAttribute('instruction');
    // if (ScratchTextHtml) {
      const ScratchOverlayWrapper = document.createElement('div');
      ScratchOverlayWrapper.classList.add(`overlay_wrapper`);

      const ScratchTextDiv = document.createElement('div');
      ScratchTextDiv.classList.add(`text_wrapper`);
      ScratchTextDiv.innerHTML = ScratchTextHtml;
      ScratchOverlayWrapper.appendChild(ScratchTextDiv);
    // }
    
    const ScratchInstructionDiv = document.createElement('div');
    ScratchInstructionDiv.classList.add(`instruction_wrapper`);
    ScratchInstructionDiv.innerHTML = `
      <span class="scratch-icon">
      ...
      <span class="scratch-instruction-text">${ScratchInstruction}</span>
    `;
    ScratchOverlayWrapper.appendChild(ScratchInstructionDiv);

    parentEl.appendChild(ScratchOverlayWrapper);
    this._ScratchTextElement = ScratchOverlayWrapper;

    requestAnimationFrame(() => {
      const textContainer = ScratchOverlayWrapper.querySelector('.instruction_wrapper .scratch-instruction-text');
      if (textContainer) {
        ...
        if (childEl) {
          ...
          const svgPaths = ScratchOverlayWrapper.querySelectorAll('.instruction_wrapper svg path');
  ...
  async startScratchCard(containerSelector, width, height, coverImageSrc, coverImageRendition, mode, clearZoneRadius) {
    try {
      ...
      if (coverImageSrc) {
        ...
        await new Promise((resolve, reject) => {
          img.onload = () => {
            offscreenCtx.fillStyle = this.getAttribute('section_bg_color') || 'transparent';
            offscreenCtx.fillRect(0, 0, width, height);
            drawImageScaledAndCentered(offscreenCtx, img, width, height, coverImageRendition);
            resolve();
          };
          img.onerror = reject;
        });
      }else {
        // No image provided, just fill with background color
        offscreenCtx.fillStyle = this.getAttribute('section_bg_color') || '#CCCCCC';
        offscreenCtx.fillRect(0, 0, width, height);
      }

      const sc = new ScratchCard(containerSelector, {
        scratchType: SCRATCH_TYPE.LINE,
        containerWidth: width,
        containerHeight: height,
        imageForwardSrc: offscreenCanvas.toDataURL(),
        imageBackgroundSrc: '',
        htmlBackground: '',
        clearZoneRadius,
        nPoints: 0,
        ...
        callback: () => {
          AdEventTracker?.TriggerEventFromWidgetConfig?.({
            ...
            eventName: `ScratchCompleted`,
            widgetTagName: 'wgt-scratch-card'
          });
      ...
      await sc.init().then(() => {
        ...
        console.log("scratch is not rendered", error); // Log the error for debugging
      });

      document.querySelector(containerSelector).classList.add(
        mode == '1' ? 'fullscreen-canvas' : 'fixed-canvas'
      );
      ...
      const hideOverlay = () => {
        // console.log('[TRACKING] Scratch started for card:', this);
        AdEventTracker?.TriggerEventFromWidgetConfig?.({
          widgetOrder: this.widgetOrder,
          widgetId: this.widgetId,
          adWidgetId: this.adWidgetId,
          eventName: `ScratchStarted`,
          widgetTagName: 'wgt-scratch-card'
        });
        this.hideScratchTextOverlay();
        canvas.removeEventListener('pointerdown', hideOverlay);
        canvas.removeEventListener('touchstart', hideOverlay);
      };

      canvas.addEventListener('pointerdown', hideOverlay, { once: true });
      canvas.addEventListener('touchstart', hideOverlay, { once: true });

      this._scratchInstance = sc;
    } catch (err) {
      console.error('[wgt-scratch-card] ScratchCard init error:', err);
    }
  }
  ...
  hideScratchTextOverlay() {
    if (this._ScratchTextElement) {
      this._ScratchTextElement.style.opacity = '0';
      this._ScratchTextElement.addEventListener('transitionend', () => {
        this._ScratchTextElement?.remove();
        this._ScratchTextElement = null;
      }, { once: true });
    }
  ...
  destroy() {
    this._scratchInstance?.canvas?.remove();
    this._scratchWrapper?.remove();
    this._ScratchTextElement?.remove();
    this.remove();
    ...
    this._ScratchTextElement = null;
    this.isRendered = false;
    this.isCompleted = false; // Reset completion status on destroy
  }
...
function drawImageScaledAndCentered(ctx, img, canvasWidth, canvasHeight, coverImageRendition) {
  const iw = img.width;
  const ih = img.height;
  let sx = 0, sy = 0, sWidth = iw, sHeight = ih;
  let dx = 0, dy = 0, dWidth = canvasWidth, dHeight = canvasHeight;

  switch (parseInt(coverImageRendition, 10)) {
    case 2: // Crop to Fill
    ...
    case 3: // Stretch to Fill
      // draw full image stretched, nothing to crop
      break;

    case 1: // Scale to Fit (default)
    default:
      const scaleFit = Math.min(canvasWidth / iw, canvasHeight / ih);
      dWidth = iw * scaleFit;
      dHeight = ih * scaleFit;
...
customElements.define('wgt-scratch-card', WgtScratchCard);
